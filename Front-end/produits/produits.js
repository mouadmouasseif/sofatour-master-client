...
$(document).ready(function() {
    ...
    if(success != null)
    {
        error_toast("success",success);
    }

    // manage product data table
	manageProductTable = $('#manageProductTable').DataTable({
		'ajax': '../Controllers/fetchProduct.php',
		'order': []
	});
	...
	$("#addProductModalBtn").unbind('click').bind('click', function() {
		...
		$("#productImage").fileinput({
	      overwriteInitial: true,
		    ...
		    defaultPreviewContent: '<img src="images/photo_default.png" alt="Profile Image" style="width:100%;">',
		...
		$("#submitProductForm").unbind('submit').bind('submit', function() {
            ...
            var quantity_initial = $("#quantity_initial").val();
            ...
            if(quantity_initial == "") {
				$("#quantity_initial").after('<p class="text-danger">quantity_initial field is required</p>');
				$('#quantity_initial').closest('.form-group').addClass('has-error');
			}	else {
				// remov error text field
			...
			if(productImage && productName && Description && quantity && rate && quantity_initial && categoryName && productStatus) {
...
function editProduct(productId = null) {
	...
	if(productId) {
		...
		$.ajax({
			url: '../Controllers/fetchSelectedProduct.php',
			type: 'post',
			...
			// alert(response.product_image);
				...
				$("#getProductImage").attr('src', 'documents/photo_produits/'+response.product_image);
		  ...
		  // 		allowedFileExtensions: ["jpg", "png", "gif", "JPG", "PNG", "GIF"]
				...
				// brand name
				$("#editQuantity_initial").val(response.quantity_initiale);
				// category name
				...
				$("#editProductForm").unbind('submit').bind('submit', function() {
					...
					var quantity_initiale = $("#editQuantity_initial").val();
					...
					if(quantity_initiale == "") {
						$("#editQuantity_initial").after('<p class="text-danger">Quantity Name field is required</p>');
						$('#editQuantity_initial').closest('.form-group').addClass('has-error');
					}	else {
						// remov error text field
						$("#editQuantity_initial").find('.text-danger').remove();
						// success out for form 
						$("#editQuantity_initial").closest('.form-group').addClass('has-success');	  	
					}	// /else
					...
					if(productName && quantity && rate && quantity_initiale && categoryName && productStatus) {
						// submit loading button
...
function removeProduct(productId = null) {
	if(productId) {
		...
		$("#removeProductBtn").unbind('click').bind('click', function() {
			...
			$.ajax({
				url: '../Controllers/removeProduct.php',
				type: 'post',
				data: {productId: productId},
				dataType: 'json',
				success:function(response) {
