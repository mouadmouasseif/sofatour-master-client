...
include 'connection_database.php';
 ...
 $devis_facture = $_GET['devis_facture'];
 $numero_devis_facture = $_GET['numero_devis_facture'];
 $id_client = $_GET['id_client'];
 $version = $_GET['version'];

 if( $devis_facture == 'Facture')
{
 $queryDevisClient = "SELECT id_client_Facture, id_client, utilisateurs , Numero_Facture, prix_total_ttc FROM client_facture_client WHERE id_client = :id_client AND Numero_Facture = :numero_devis_facture ";
 $stmtDevisClient = $conn->prepare($queryDevisClient);
 $stmtDevisClient->bindParam(':id_client',  $id_client , PDO::PARAM_INT);
 $stmtDevisClient->bindParam(':numero_devis_facture',  $numero_devis_facture , PDO::PARAM_STR);
}
else
{
    $queryDevisClient = "SELECT id_client_devis , id_client, utilisateurs, version_devis, Numero_devis, prix_total_ttc FROM client_devis_client WHERE id_client = :id_client AND Numero_devis = :numero_devis_facture AND version_devis = :versions";
    $stmtDevisClient = $conn->prepare($queryDevisClient);
    $stmtDevisClient->bindParam(':id_client',  $id_client , PDO::PARAM_INT);
    $stmtDevisClient->bindParam(':numero_devis_facture',  $numero_devis_facture , PDO::PARAM_STR);
...
//  var_dump($client_devis_client['id_client_devis']);
  ...
  if($devis_facture == 'Facture')
  {
    $queryDevis = "SELECT le_devis, a_devis, objet, date_d_entree, du_date, a_tel_date, TVA , devis_objet FROM client_devis WHERE id_client_devis = :id_client_Facture";
    $stmtDevis = $conn->prepare($queryDevis);
    $stmtDevis->bindParam(':id_client_Facture',  $client_devis_client['id_client_Facture']  , PDO::PARAM_INT);
    $stmtDevis->execute();
    ...
    $stmtLigneDevis->bindParam(':client_devis', $client_devis_client['id_client_Facture'], PDO::PARAM_INT);
    ...
    $stmtLigneDevis2->bindParam(':client_devis', $client_devis_client['id_client_Facture'], PDO::PARAM_INT);
    ...
    $stmtLigneDevis3->bindParam(':client_devis', $client_devis_client['id_client_Facture'], PDO::PARAM_INT);
  ...
  $responseData = array(
      ...
      'client_ligne_devis_3' =>   $client_ligne_devi3,
  );
