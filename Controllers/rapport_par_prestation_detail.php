...
if (isset($postData['annee']) && isset($postData['prestation']) &&  isset($postData['DeFa']) ) {
    ...
    if( $DeFa  == 'D')
    {
        $query = "SELECT (cld.pt_ht + (cld.pt_ht * (cd.TVA/100))) AS total_prix, ldp.designation , ldp.prestation , ldp.id_ligne_devis_prestation, cdc.Numero_devis, c.nom_complet, c.societe, cd.du_date, cd.a_tel_date
                  FROM client_devis_client cdc
                  JOIN clients c ON c.id_client = cdc.id_client
                  ...
                  WHERE EXTRACT(YEAR FROM cd.du_date) = ? AND confirmer = 1 AND annuler = 0 AND ldp.id_ligne_devis_prestation = ?
                  ORDER BY total_prix DESC";
    }
    else if( $DeFa  == 'F')
    {
        $query = "SELECT (cld.pt_ht + (cld.pt_ht * (cd.TVA/100))) AS total_prix, ldp.designation , ldp.prestation , ldp.id_ligne_devis_prestation, cdc.Numero_Facture as Numero_devis, c.nom_complet, c.societe, cd.du_date, cd.a_tel_date
                  FROM client_facture_client cdc
                  JOIN clients c ON c.id_client = cdc.id_client
                  JOIN client_devis cd ON cd.id_client_devis = cdc.id_client_facture
                  JOIN client_ligne_devis cld ON cld.client_devis = cd.id_client_devis
                  JOIN ligne_devis_prestation ldp ON ldp.id_ligne_devis_prestation = cld.ligne_devis_prestation
                  WHERE EXTRACT(YEAR FROM cd.du_date) = ? AND ldp.id_ligne_devis_prestation = ?
                  ORDER BY total_prix DESC;";
    }
